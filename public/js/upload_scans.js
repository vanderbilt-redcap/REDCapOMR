$(document).ready(function() {
        ...
        $.ajax({
            ...
            success: function(response) {
                ...
                if($.trim(response.error)) {
                    ...
                    OMR_ProjectVars.addScans = document.getElementById('addScans');
            ...
            error: function(response) {
                ...
                OMR_ProjectVars.addScans = document.getElementById('addScans');
                OMR_ProjectVars.addScans.innerHTML = '';
    ...
    $('#upload').on('click', function() {
        ...
        OMR_ProjectVars.ajaxData = new FormData($('#form')[0]);
        //Iterate through files and append all to data
        $.each($("input[type=file]"), function(i, obj) {
            $.each(obj.files,function(j, file){
                OMR_ProjectVars.ajaxData.append('upload['+j+']', file);
            });
        });
        //Add the selected instrument to the form data
        OMR_ProjectVars.ajaxData.append('instruments', document.getElementById('instruments').value);

        $.ajax({
            type: "POST",
            url: "../functions/upload_scan_func.php",
            data: OMR_ProjectVars.ajaxData,
            dataType: "text",
            processData: false,
            contentType: false,
            beforeSend: function() {
